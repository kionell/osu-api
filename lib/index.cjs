...
class APIClient {
  ...
  async _request(config) {
    try {
      const hash = md5__default['default'](JSON.stringify(config));
...
function searchBeatmap(beatmapsets, query) {
  ...
  for (const beatmapset of beatmapsets) {
    ...
    for (const beatmap of beatmaps) {
      ...
      if (hasAllKeywords(version, keywords)) {
        return beatmap;
      }

      if (hasAllKeywords(creator, keywords)) {
        return beatmap;
      }

      if (hasAllKeywords(title, keywords)) {
        return beatmap;
      }

      if (hasAllKeywords(artist, keywords)) {
        return beatmap;
      }
...
exports.getRulesetShortname = getRulesetShortname;
