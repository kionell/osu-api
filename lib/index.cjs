...
let axios__default = /* #__PURE__*/_interopDefaultLegacy(axios);
...
class APIClient {
  ...
  static getInstance() {
    if (!this._clientInstance) {
      const APIClient = this.constructor;
  ...
  async _request(url, method, data) {
    const hash = md5__default['default'](url);
...
function getRuleset(input) {
  ...
  switch (rulesetId) {
    case 0: return new osuStandardStable.StandardRuleset();
    case 1: return new osuTaikoStable.TaikoRuleset();
    case 2: return new osuCatchStable.CatchRuleset();
    case 3: return new osuManiaStable.ManiaRuleset();
  }
}
...
class BanchoBeatmapInfo extends osuClasses.BeatmapInfo {
  constructor(other) {
    super();
...
class BanchoCatchDifficultyAttributes extends osuCatchStable.CatchDifficultyAttributes {
  constructor(other, mods) {
    const ruleset = new osuCatchStable.CatchRuleset();
...
class BanchoManiaDifficultyAttributes extends osuManiaStable.ManiaDifficultyAttributes {
  constructor(other, mods) {
    const ruleset = new osuManiaStable.ManiaRuleset();
    const combination = ruleset.createModCombination(mods);
...
class BanchoOsuDifficultyAttributes extends osuStandardStable.StandardDifficultyAttributes {
  constructor(other, mods) {
    const ruleset = new osuStandardStable.StandardRuleset();
    const combination = ruleset.createModCombination(mods);
...
class BanchoScoreInfo extends osuClasses.ScoreInfo {
  constructor(other) {
    super();
    this.id = other.id;
...
class BanchoTaikoDifficultyAttributes extends osuTaikoStable.TaikoDifficultyAttributes {
  constructor(other, mods) {
    const ruleset = new osuTaikoStable.TaikoRuleset();
    const combination = ruleset.createModCombination(mods);
...
class BanchoUserInfo extends osuClasses.UserInfo {
  constructor(other) {
    super();
...
class BanchoAPIClient extends APIClientWithOAuth {
  ...
  async authorize(clientId, clientSecret) {
    ...
    try {
      const res = await axios__default['default'].post(url, data, this.config);
...
class BanchoURLScanner extends URLScanner {
  constructor() {
    ...
    this.BEATMAP_REGEX = new RegExp(''
            ...
            + /\/(b|beatmaps)/.source
            + /\/[0-9]+/.source
            + /(\?mode=(osu|taiko|fruits|mania)){0,1}$/.source);
  ...
  getRulesetIdFromURL(url) {
    if (url?.includes('taiko')) {
      return 1;
    }

    if (url?.includes('fruits')) {
      return 2;
    }

    if (url?.includes('mania')) {
      return 3;
    }
...
class APIFactory {
  getAPIClient(server = 0) {
    switch (server) {
      case 2:
      case 3:
      case 1:
      case 0: return BanchoAPIClient.getInstance();
    }

    throw new Error('This server is not found or not supported!');
  }
  createURLScanner(server = 0) {
    switch (server) {
      case 2:
      case 3:
      case 1:
      case 0: return new BanchoURLScanner();
    }

    throw new Error('This server is not found or not supported!');
  }
  createURLGenerator(server = 0) {
    switch (server) {
      case 2:
      case 3:
      case 1:
      case 0: return new BanchoURLGenerator();
    }
...
exports.APIClientWithOAuth = APIClientWithOAuth;
exports.APIFactory = APIFactory$1;
exports.AuthTokens = AuthTokens;
exports.BanchoAPIClient = BanchoAPIClient;
exports.BanchoAuthTokens = BanchoAuthTokens;
exports.BanchoBeatmapInfo = BanchoBeatmapInfo;
exports.BanchoCatchDifficultyAttributes = BanchoCatchDifficultyAttributes;
exports.BanchoManiaDifficultyAttributes = BanchoManiaDifficultyAttributes;
exports.BanchoOsuDifficultyAttributes = BanchoOsuDifficultyAttributes;
exports.BanchoScoreInfo = BanchoScoreInfo;
exports.BanchoTaikoDifficultyAttributes = BanchoTaikoDifficultyAttributes;
exports.BanchoURLGenerator = BanchoURLGenerator;
exports.BanchoURLScanner = BanchoURLScanner;
exports.BanchoUserInfo = BanchoUserInfo;
...
exports.searchBeatmap = searchBeatmap;
exports.sortUserBest = sortUserBest;
